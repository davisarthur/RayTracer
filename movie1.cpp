...
int main() {
   ...
   // Create the image (RGB Array) to be displayed
    ...
    // camera settings
    Vector3 viewDir(0.0, -0.2, -1.0), up(0.0, 1.0, 0.0), viewPoint(0.0, 10.0, 20.0);
   ...
   for (int n = 0; n < fps * dur; n++) {
      ...
      // reorient camera
      float viewX = cos(time * 2.0 * M_PI / (float) period + M_PI / 3.0) / pow(1.0 + pow(0.2, 2), 0.5f);
      float viewY = -0.2f;
      float viewZ = -sin(time * 2.0 * M_PI / (float) period + M_PI / 3.0) / pow(1.0 + pow(0.2, 2), 0.5f);
      Vector3 newViewDir(viewX, viewY, viewZ);

      // create and render scene
      scene.cam->changeOrientation(viewPoint, up, newViewDir);
      ...
      */
      ...
      std::string fnameTemp = "movie1/img" + std::to_string(n) + ".png";
